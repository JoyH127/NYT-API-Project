...
def news_extraction(url):
    ...
    # if there is no subreddit passing, it will set.
    # worldnews reddit as a default.
    if url is None:
...
def gun_in_articles_cvs(title):
    ...
    # and 'Gun Count'.
    # I added up the newline="\n" to remove empty line each row.
    count = 0
    with open("gunresult.csv", "w", newline="\n") as csvfile:
...
gun_in_articles_cvs(get_lowercase_titles(news_extraction('https://www.reddit.com/r/news/.json')))
